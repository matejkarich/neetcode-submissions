class Twitter:
    ...
    def __init__(self):
        ...
        self.userUnfollowed = defaultdict(list)
        self.time = 0
    ...
    def postTweet(self, userId: int, tweetId: int) -> None: # O(log(10)) -> O(1)
        ...
        if len(userTweetHeap) < 10:
            ...
            heapq.heappush_max(userTweetHeap, (self.time, tweetId))
            oldestTweetIndex = userTweetHeap.index(min(userTweetHeap)) # O(10)
            userTweetHeap[oldestTweetIndex] = userTweetHeap[-1]
            userTweetHeap.pop()
            heapq.heapify_max(userTweetHeap) # O(10)
        self.time += 1
    ...
    def getNewsFeed(self, userId: int) -> List[int]:
        ...
        while len(feed) < 10: # O(10 * (f*u + 1) + 10 * log(10)) -> O(f*u) where f is # of people user is following
            if userTweets:
                mostRecentTweetHeap = userTweets
            else:
                mostRecentTweetHeap = None
            for f in self.userFollowing[userId]:
                if f in self.userUnfollowed[userId]:
                    continue
    ...
    def unfollow(self, followerId: int, followeeId: int) -> None: # O(1)
        self.userUnfollowed[followerId].append(followeeId)
