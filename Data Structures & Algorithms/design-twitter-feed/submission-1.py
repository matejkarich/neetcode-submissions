class Twitter:
    ...
    def postTweet(self, userId: int, tweetId: int) -> None: # O(log(10)) -> O(1)
        ...
        if len(userTweetHeap) < 10:
            ...
            heapq.heappushpop_max(userTweetHeap, (self.time, tweetId))
        self.time += 1
    ...
    def getNewsFeed(self, userId: int) -> List[int]:
        ...
        while len(feed) < 10: # O(10 * (f + 1) + 10 * log(10)) -> O(f) where f is # of followers for user
            if userTweets:
                mostRecentTweetHeap = userTweets
            else:
                mostRecentTweetHeap = None
            for f in self.userFollowing[userId]:
                if self.userTweets[f] and self.userTweets[f][0][0] > mostRecentTweetHeap[0][0]:
    ...
    def unfollow(self, followerId: int, followeeId: int) -> None: # O(f) where f is # of followers for user
